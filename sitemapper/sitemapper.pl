...
JAVASCRIPT_FOOTER
    ...
    'html'        => sub {
		print "<h2>Total number of HTML pages: ";
		print shift;
		print "</h2>";
		print "<h2>Total number of non-HTML documents: ";
		print shift;
		print "</h2>";
		print "<h2>Total number of folders: ";
		print shift;
		print "</h2>";
		print "<h2>Total size: ";
		print shift;
...
sub print_sitemap()
{
	...
	my $total_html_count = 0;
	my $total_folder_count = 1;
	my $total_non_html_count = 0;
	my $total_size = 0;
	my @visited_folders = ( $sitemap->root() );
    ...
    $sitemap->traverse(
        sub {
            ...
            elsif( $flag == 1 )
            {
                ...
                my $size = $sitemap->size( $url );
                if ( !$opt_html_only or $sitemap->is_html_file( $url ) )
                {
                    $print_node{ $opt_format }->( $url, $depth, $title, $summary, $size );
                }
				my $new_folder = 1;
				for (@visited_folders)
				{
					if ( $_ eq substr($url, 0, rindex($url, '/')) )
					{
						$new_folder = 0;
					}
				}
				if ($new_folder == 1)
				{
					push(@visited_folders, substr($url, 0, rindex($url, '/')));
					$total_folder_count++;
				}
    ...
    $print_page_end{ $opt_format }->( $total_html_count, $total_non_html_count, $total_folder_count, int($total_size / 1024));
}
