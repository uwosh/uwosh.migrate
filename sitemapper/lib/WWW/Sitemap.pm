...
=head2 is_html_file( $url )

Returns 1 if $url points to an html file (i.e. *.php or *.html).

    if ( $sitemap->is_html_file( $url ) )
    {
        # do something...
    }
    
=head2 is_document( $url )

Returns 1 if $url points to any kind of document.

    if ( $sitemap->is_document( $url ) )
    {
        # do something...
    }
...
#------------------------------------------------------------------------------
#
# is_html_file - returns TRUE if $url points to an html file, FALSE otherwise
#
#------------------------------------------------------------------------------
...
sub is_html_file
{
    my $self = shift;
    my $url = shift;
    return (($url =~ m{(?:/|\.s?html?)$}) || ($url =~ m{(?:/|\.s?php?)$}));
}

#------------------------------------------------------------------------------
#
# is_document - returns TRUE if $url points to a document that 
# cannont be indexed, FALSE otherwise
#
#------------------------------------------------------------------------------
...
sub is_document
{
    my $self = shift;
    my $url = shift;
    return (($url =~ m{(?:/|\.s?pdf?)$}) || ($url =~ m{(?:/|\.s?doc?)$}) ||
            ($url =~ m{(?:/|\.s?html?)$}) || ($url =~ m{(?:/|\.s?php?)$}));
}
...
#------------------------------------------------------------------------------
#
# size - returns the size of a given URL
#
#------------------------------------------------------------------------------
...
sub generate
{
    ...
    $self->{ 'robot' }->addHook( 
        ...
        sub {
            ...
            return unless $self->is_document( $to_url );
    ...
    $self->{ 'robot' }->addHook( 
        ...
        sub {
            ...
            return 0 unless $self->is_document( $url );
            # check the current depth, if the DEPTH option is set
            return 0 if (
                defined $self->{ DEPTH } and
    ...
    $self->{ 'robot' }->addHook( 
        ...
        sub {
            my( $robot, $hook, $url ) = @_;
            if ( !$self->is_html_file( $url ) )
            {
                $self->{ 'urls' }{ $url }++;
                $self->{ 'title' }{ $url } = substr($url, rindex($url, '/') + 1);
                $self->{ 'summary' }{ $url } = 'Document';
            }
            my $size = HTTP::Size::get_size( $url );
            if (defined $size)
            {
                $self->{ 'size' }{ $url } = int($size / 1024) or 1;
            }
            else 
            {
                 $self->{ 'size' }{ $url } = 0;
